...
static void observer_begin(zend_execute_data *execute_data, zend_llist *hooks) {
    ...
    for (zend_llist_element *element = hooks->head; element;
        ...
        if (!is_valid_signature(fci, fcc)) {
            php_error_docref(NULL, E_WARNING, "OpenTelemetry: pre hook invalid signature, "
                "class=%s function=%s", (Z_TYPE_P(&params[2]) == IS_NULL) ? "null" : Z_STRVAL_P(&params[2]), Z_STRVAL_P(&params[3]));
            continue;
        }
                ...
                ZEND_HASH_FOREACH_KEY_VAL(Z_ARR(ret), idx, str_idx, val) {
                    ...
                    if (idx >= arg_count) {
                        if (func->type == ZEND_INTERNAL_FUNCTION) {
                            ...
                            // segfault
                            php_error_docref(NULL, E_NOTICE, "OpenTelemetry: expanding args of "
...
static void observer_end(zend_execute_data *execute_data, zval *retval,
    ...
    for (zend_llist_element *element = hooks->tail; element;
        ...
        if (!is_valid_signature(fci, fcc)) {
            php_error_docref(NULL, E_WARNING, "OpenTelemetry: pre hook invalid signature, "
                "class=%s function=%s", (Z_TYPE_P(&params[4]) == IS_NULL) ? "null" : Z_STRVAL_P(&params[4]), Z_STRVAL_P(&params[5]));
            continue;
        }
