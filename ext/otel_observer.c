...
static void observer_end(zend_execute_data *execute_data, zval *retval,
    ...
    for (zend_llist_element *element = hooks->tail; element;
        ...
        if (zend_call_function(&fci, &fcc) == SUCCESS) {
            // TODO rather than ignoring return value if post callback doesn't have a return type-hint,
            //      could we check whether the types are compatible and allow if they are?
            if (!Z_ISUNDEF(ret) &&
