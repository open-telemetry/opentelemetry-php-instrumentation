...
function install_conf_d_files() {
    PHP_CONFIG_D_PATH=$1
    INI_FILE_PATH="${EXTENSION_CFG_DIR}/$OPENTELEMETRY_INI_FILE_NAME"
    ...
    echo "Configuring ${OPENTELEMETRY_INI_FILE_NAME} for supported SAPI's"
    ...
    for SAPI_CONFIG_D_PATH in "${SAPI_CONFIG_DIRS[@]}" ; do
        ...
        link_file "${INI_FILE_PATH}" "${SAPI_CONFIG_D_PATH}/98-${OPENTELEMETRY_INI_FILE_NAME}"
...
; THIS IS AN AUTO-GENERATED FILE by the OpenTelemetry auto-instrumentation extension post-install.sh script
; To overwrite the INI settings for this extension, edit
; the INI file in this directory "${CUSTOM_INI_FILE_PATH}"
[opentelemetry]
${CONTENT}
; END OF AUTO-GENERATED by the OpenTelemetry auto-instrumentation extension post-install.sh script
EOF
...
################################################################################
#### Function manual_extension_instrumentation_setup #####################################
function manual_extension_instrumentation_setup() {
    echo 'Set up the instrumentation manually as explained in:'
...
################################################################################
#### Function instrumentation_extension_not_supported ####################################
function instrumentation_extension_not_supported() {
    PHP_API=$(php_api)
    echo 'Failed. OpenTelemetry auto-instrumentation extension extension not supported for the current PHP API version.'
...
#        SUFFIX=-alpine
#    fi
    echo "${EXTENSION_DIR}/opentelemetry-${PHP_API}${SUFFIX}.so"
}
...
function is_php_supported() {
    ...
    # Make sure list of PHP versions supported by the OpenTelemetry auto-instrumentation extension is in sync.
    ...
    then
        ...
        echo 'Failed. The supported PHP versions are 8.0-8.2.'
        return 1
    fi
}
...
############################### MAIN ###########################################
################################################################################
echo 'Installing OpenTelemetry auto-instrumentation extension'
...
if ! is_php_supported ; then
    echo 'Failed. OpenTelemetry auto-instrumentation extension extension is not supported for the existing PHP installation.'
    exit 1
fi
    ...
    if [ -e "${PHP_INI_FILE_PATH}" ] ; then
        if [ -e "${EXTENSION_FILE_PATH}" ] ; then
            if grep -q "${EXTENSION_FILE_PATH}" "${PHP_INI_FILE_PATH}" ; then
                echo '  extension configuration already exists for the OpenTelemetry auto-instrumentation extension.'
                echo '  skipping ... '
            else
                echo "${PHP_INI_FILE_PATH} has been configured with the OpenTelemetry auto-instrumentation extension setup."
                cp -fa "${PHP_INI_FILE_PATH}" "${PHP_INI_FILE_PATH}${BACKUP_EXTENSION}"
                add_extension_configuration_to_file "${PHP_INI_FILE_PATH}"
            fi
        else
            instrumentation_extension_not_supported
        fi
    else
        if [ -e "${EXTENSION_FILE_PATH}" ] ; then
            echo "${PHP_INI_FILE_PATH} has been created with the OpenTelemetry auto-instrumentation extension setup."
            add_extension_configuration_to_file "${PHP_INI_FILE_PATH}"
        else
            instrumentation_extension_not_supported
        fi
...
if is_extension_enabled ; then
    echo 'Extension enabled successfully for OpenTelemetry auto-instrumentation extension'
else
    echo 'Failed. OpenTelemetry auto-instrumentation extension extension is not enabled'
    ...
    manual_extension_instrumentation_setup
fi
